...
class OrientDBCommandRecordUpdate extends OrientDBCommandAbstract
{
     ...
     * Record type
     * @var string
     * @see \Gratheon\OrientDB\OrientDB::$recordTypes
     */
     ...
     * @var string|\Gratheon\OrientDB\OrientDBRecord
     */
    ...
    public function prepare()
    {
        ...
        if (count($this->attribs) > 4 || count($this->attribs) < 2) {
            throw new \Gratheon\OrientDB\OrientDBWrongParamsException('This command requires record ID, record content and, optionally, record version and, optionally, record type');
        }
        $arr = explode(':', $this->attribs[0]);
        if (count($arr) != 2) {
            throw new \Gratheon\OrientDB\OrientDBWrongParamsException('Wrong format for record ID');
        }
        ...
        if (count($this->attribs) == 4) {
            if (in_array($this->attribs[3], \Gratheon\OrientDB\OrientDB::$recordTypes)) {
                $this->recordType = $this->attribs[3];
            } else {
                throw new \Gratheon\OrientDB\OrientDBWrongParamsException('Incorrect record Type: ' . $this->attribs[2] . '. Available types is: ' . implode(', ', \Gratheon\OrientDB\OrientDB::$recordTypes));
            }
        } else {
            $this->recordType = \Gratheon\OrientDB\OrientDB::RECORD_TYPE_DOCUMENT;
        }
        // Add recordType
    ...
    protected function parseResponse()
    {
        ...
        if ($this->recordContent instanceof \Gratheon\OrientDB\OrientDBRecord) {
