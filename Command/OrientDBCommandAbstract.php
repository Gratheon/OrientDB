...
abstract class OrientDBCommandAbstract
{
     ...
     * @var \Gratheon\OrientDB\OrientDBSocket
     */
    private $socket;
     ...
     * @var \Gratheon\OrientDB\OrientDB
     */
    protected $parent;

    /**
     * Construct new instance
     * @param \Gratheon\OrientDB\OrientDB $parent
     */
     ...
     * @throws \Gratheon\OrientDB\OrientDBWrongCommandException
     */
    public function prepare()
    {
        ...
        if ($this->opType === self::DB_OPEN || $this->opType === self::CONNECT) {
            ...
            if (in_array($this->opType, $this->parent->getCommandsRequiresConnect())) {
                ...
                throw new \Gratheon\OrientDB\OrientDBWrongCommandException('Unknown command');
            }
        }
     ...
     * @throws null|\Gratheon\OrientDB\OrientDBException
     * @throws \Gratheon\OrientDB\OrientDBException
     * @return mixed
     */
    public function execute()
    {
        ...
        if ($requestTransactionID !== $this->currentTransactionID) {
            throw new \Gratheon\OrientDB\OrientDBException('Transaction ID mismatch');
        }
        ...
        if ($this->requestStatus === chr(OrientDBCommandAbstract::STATUS_SUCCESS)) {
            ...
            while ($this->readByte() === chr(OrientDBCommandAbstract::STATUS_ERROR)) {
                ...
                $exception = new \Gratheon\OrientDB\OrientDBException($javaException . ': ' . $javaExceptionDescr, 0, is_null($exception) ? null : $exception);
            }
            throw $exception;
        } else {
            throw new \Gratheon\OrientDB\OrientDBException('Unknown error');
        }
    }
    ...
    protected function readRaw($length)
    {
        ...
        do {
			...
			$delta = time() - $time;
			$timeoutReached = $this->timeout > 0 && ($delta > $this->timeout);
        } while ($dataLeft > 0 && !$timeoutReached);

		if($timeoutReached){
			throw new \Gratheon\OrientDB\OrientDBConnectException("Timeout of ".$this->timeout."sec reached");
		}
        return $data;
    }
     ...
     * @return boolean|\Gratheon\OrientDB\OrientDBTypeLink|OrientDBRecord
     */
    protected function readRecord()
    {
        ...
        if ($marker == -3) {
            ...
            return new \Gratheon\OrientDB\OrientDBTypeLink($clusterID, $recordPos);
        }
    ...
    {
        if ($string instanceof \Gratheon\OrientDB\OrientDBRecord) {
    ...
    public static function unpackI64($hi, $low)
    {
        ...
        if (!extension_loaded('bcmath')) {
            throw new \Gratheon\OrientDB\OrientDBException('Required bcmath module to continue');
        }
