...
class OrientDBCommandDataclusterCount extends OrientDBCommandAbstract
{
    ...
    public function prepare()
    {
        ...
        if (count($this->attribs) != 1) {
            throw new \Gratheon\OrientDB\OrientDBWrongParamsException('This command requires cluster IDs in array');
        }
        if (!is_array($this->attribs[0])) {
            throw new \Gratheon\OrientDB\OrientDBWrongParamsException('Array expected');
        }
        // Add count
