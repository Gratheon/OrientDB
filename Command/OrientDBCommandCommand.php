...
class OrientDBCommandCommand extends OrientDBCommandAbstract
{
    ...
    public function prepare()
    {
        ...
        if (count($this->attribs) > 3 || count($this->attribs) < 2) {
            throw new \Gratheon\OrientDB\OrientDBWrongParamsException('This command requires query and, optionally, fetchplan');
        }
        if (empty($this->mode)) {
            if ($this->attribs[0] == \Gratheon\OrientDB\OrientDB::COMMAND_SELECT_SYNC || $this->attribs[0] == \Gratheon\OrientDB\OrientDB::COMMAND_SELECT_ASYNC || $this->attribs[0] == \Gratheon\OrientDB\OrientDB::COMMAND_QUERY) {
                $this->mode = $this->attribs[0];
            } else {
                throw new \Gratheon\OrientDB\OrientDBWrongParamsException('Wrong command mode');
            }
        }
        if (($this->mode == \Gratheon\OrientDB\OrientDB::COMMAND_QUERY || $this->mode == \Gratheon\OrientDB\OrientDB::COMMAND_SELECT_SYNC) && count($this->attribs) == 3) {
            throw new \Gratheon\OrientDB\OrientDBWrongParamsException('Fetch is useless with COMMAND_QUERY');
        }
        ...
        // Add mode
        if ($this->mode == \Gratheon\OrientDB\OrientDB::COMMAND_QUERY || $this->mode == \Gratheon\OrientDB\OrientDB::COMMAND_SELECT_SYNC) {
            $this->addByte(self::MODE_SYNC);
        } else {
            $this->addByte(self::MODE_ASYNC);
        }
        if ($this->mode == \Gratheon\OrientDB\OrientDB::COMMAND_SELECT_ASYNC) {
            $objName = 'com.orientechnologies.orient.core.sql.query.OSQLAsynchQuery';
        } elseif ($this->mode == \Gratheon\OrientDB\OrientDB::COMMAND_SELECT_SYNC) {
        ...
        $buff .= $this->query;
        if ($this->mode == \Gratheon\OrientDB\OrientDB::COMMAND_SELECT_ASYNC || $this->mode == \Gratheon\OrientDB\OrientDB::COMMAND_SELECT_SYNC) {
     ...
     * @return bool|\Gratheon\OrientDB\OrientDBTypeLink|\Gratheon\OrientDB\OrientDBRecord|string|array
     */
