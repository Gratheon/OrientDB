...
class OrientDBCommandDataclusterRemove extends OrientDBCommandAbstract
{
    ...
    public function prepare()
    {
        ...
        if (count($this->attribs) != 1) {
            throw new \Gratheon\OrientDB\OrientDBWrongParamsException('This command requires cluster ID');
        }
        if (is_int($this->attribs[0])) {
            $this->clusterID = $this->attribs[0];
        } else {
            throw new \Gratheon\OrientDB\OrientDBWrongParamsException('Integer expected');
        }
        // Add clusterID
