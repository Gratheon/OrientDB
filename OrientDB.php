 ...
 * @method int recordCreate() recordCreate(int $clusterID, string $recordContent, string $recordType = \Gratheon\OrientDB\OrientDB::RECORD_TYPE_DOCUMENT) Create a new record
 ...
 * @method int recordUpdate() recordUpdate(string $recordID, string $recordContent, int $recordVersion = -1, string $recordType = \Gratheon\OrientDB\OrientDB::RECORD_TYPE_DOCUMENT) Update a record
...
class OrientDB
{
    ...
    private static $requireConnect = array(
        Command\OrientDBCommandAbstract::SHUTDOWN,
		Command\OrientDBCommandAbstract::DB_CREATE,
		Command\OrientDBCommandAbstract::DB_DELETE,
		Command\OrientDBCommandAbstract::DB_EXIST,
		Command\OrientDBCommandAbstract::CONFIG_GET,
		Command\OrientDBCommandAbstract::CONFIG_SET,
		Command\OrientDBCommandAbstract::CONFIG_LIST,
		Command\OrientDBCommandAbstract::DB_LIST);
    ...
    private static $requireDBOpen = array(
		Command\OrientDBCommandAbstract::DB_CLOSE,
		Command\OrientDBCommandAbstract::DATACLUSTER_ADD,
		Command\OrientDBCommandAbstract::DATACLUSTER_REMOVE,
		Command\OrientDBCommandAbstract::DATACLUSTER_COUNT,
		Command\OrientDBCommandAbstract::DATACLUSTER_DATARANGE,
		Command\OrientDBCommandAbstract::DATASEGMENT_ADD,
		Command\OrientDBCommandAbstract::DATASEGMENT_REMOVE,
		Command\OrientDBCommandAbstract::RECORD_LOAD,
		Command\OrientDBCommandAbstract::RECORD_CREATE,
		Command\OrientDBCommandAbstract::RECORD_UPDATE,
		Command\OrientDBCommandAbstract::RECORD_DELETE,
		Command\OrientDBCommandAbstract::COUNT,
		Command\OrientDBCommandAbstract::COMMAND,
		Command\OrientDBCommandAbstract::TX_COMMIT);
    ...
    public function __construct($host, $port = 2424, $timeout = 30)
    {
        $this->host = $host;
        $this->port = $port;
        $this->socket = new \Gratheon\OrientDB\OrientDBSocket($host, $port, $timeout);
    }
    ...
    {
        $className = '\Gratheon\OrientDB\Command\OrientDBCommand' . ucfirst($name);
        if (class_exists($className)) {
            /**
             * @var \Gratheon\OrientDB\Command\OrientDBCommandAbstract
             */
            ...
            if ($command->opType == Command\OrientDBCommandAbstract::CONNECT) {
                $this->connected = true;
            }
            if ($command->opType == Command\OrientDBCommandAbstract::DB_OPEN) {
                $this->DBOpen = true;
            }
            if ($command->opType == Command\OrientDBCommandAbstract::DB_CLOSE) {
                $this->DBOpen = false;
                $this->active = false;
                $this->socket = null;
            }
     ...
     * @param \Gratheon\OrientDB\Command\OrientDBCommandAbstract $command Command instance
     * @throws OrientDBWrongCommandException
...
//            switch ($classToken) {
//                case 'Command':
//                    $fileName = 'Command/' . $className . '.php';
//                break;
//                case 'Type':
